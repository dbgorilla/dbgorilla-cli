...
# the deployment name — including the five secrets the CLI creates before
...
locals {
  secret_ids = [
    ...
    "${local.name}-provisioning-api-key",
  ]
}
...
locals {
  startup_script = <<-EOT
    ...
    IC_PROVISIONING_API_KEY=$(retry 30 secret "${local.name}-provisioning-api-key")
    export DBG_SERVER_SECRET DBG_DB_PASSWORD INSTACLUSTR_API_KEY IC_PROMETHEUS_API_KEY IC_PROVISIONING_API_KEY
