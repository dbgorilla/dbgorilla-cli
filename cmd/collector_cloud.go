...
// printSecretPresence prints each secret parameter as set or not, never its
// value — derived as a boolean so no code path can print a credential.
func printSecretPresence(keys []string, secrets map[string]string) {
	for _, k := range keys {
		v := "(not set)"
		if secrets[k] != "" {
			v = "<redacted>"
