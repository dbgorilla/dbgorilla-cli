...
// Endpoints the deployment does not advertise fall back to the collector's built-in defaults, which
// are the PRODUCTION control plane. Against a dev or PR deployment that is almost never what the
// operator wants, and the failure it produces — a 401 from a host they never named — says nothing
// about the cause. The install has to say so out loud.
